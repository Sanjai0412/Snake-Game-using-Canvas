class Food {
  ...
  createFood(snakeBody) {
    do {
      this.foodAxis.x =
        Math.floor((Math.random() * this.canva.width) / this.size) * this.size;
      this.foodAxis.y =
        Math.floor((Math.random() * this.canva.height) / this.size) * this.size;
    } while (
      snakeBody.some((s) => s.x == this.foodAxis.x && s.y == this.foodAxis.y)
    );
  }
  displayFood() {
    ...
    this.context.arc(
      this.foodAxis.x + this.size / 2,
      this.foodAxis.y + this.size / 2,
      this.size / 2,
      0,
      Math.PI * 2,
      false
...
class Snake {
  ...
  hasHittedWall() {
    let head = this.snake[this.snake.length - 1];
    return (
      head.x >= this.canva.width ||
      head.y >= this.canva.height ||
      head.x < 0 ||
      head.y < 0
    );
  }
...
function handleTouchDirection(direction) {
  if (directionChanged || !started || paused) return;
  if (direction === "left" && snake.xVel == 0) {
    snake.setDirection(-speed, 0);
    directionChanged = true;
  }
  if (direction === "right" && snake.xVel == 0) {
    snake.setDirection(speed, 0);
    directionChanged = true;
  }
  if (direction === "up" && snake.yVel == 0) {
    snake.setDirection(0, -speed);
    directionChanged = true;
  }
  if (direction === "down" && snake.yVel == 0) {
    snake.setDirection(0, speed);
    directionChanged = true;
  }
}
...
gameBoard.addEventListener(
  "touchstart",
  function (event) {
    event.preventDefault(); // Prevents screen scrolling
